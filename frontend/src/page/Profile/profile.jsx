...
function Profile({ handleChangeImg }) {
  ...
  useEffect(() => {
    userInstance.get(`GetUserById/${userId}`).then((res) => {
    ...
    <>
      <Row className="mx-0 mt-3">
        ...
        <Col md={6}>
          ...
          <section id="switch" className="mt-4 rounded">
            <div className="btn-switch d-flex flex-row justify-content-between">
              <div className="d-flex align-items-center tab">
                {user.role == "Member" && (
                  <button
                    class={`height-50 m-0 p-3 ${
                      tab === "degree" ? "active" : ""
                    }`}
              ...
              <div className="d-flex align-item-center action">
                {user.role === "Member" && tab === "degree" && (
                <DegreePu/>
                )}
                {user.role === "Member" && tab === "degree" && (
                  <button
                    class={` m-0 btn-success-soft ${
                      tab === "degree" ? "active" : ""
                    }`}
                    onClick={() => hanldeSetTab("degree")}
                  >
